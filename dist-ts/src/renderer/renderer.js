...
if (!grid || !count || !title || !search)
    throw new Error('AllTools renderer markup is incomplete');
...
function getVisibleTools() {
    return tools.filter((tool) => {
        ...
        if (activeView === 'favorites')
            return matchesQuery && matchesCategory && Boolean(tool.favorite);
        if (activeView === 'recent')
            return matchesQuery && matchesCategory && Boolean(tool.recent);
...
void window.alltools.catalog.list().then((catalog) => {
    tools = normalizedCatalog(catalog);
    render();
}).catch(() => render());
